  ...
  var debug = true,
      ...
      scApiUrl = function(url, apiKey) {
        var resolver = ( secureDocument || (/^https/i).test(url) ? 'https' : 'http') + '://api.' + domain + '/resolve?url=',
            params = 'format=jsonp&consumer_key=' + apiKey +'&callback=?';
...
//.then(console.log())
  ...
  var audioEngine = function() {
    ...
    var html5Driver = function() {
     ...
     // $('<div class="sc-player-engine-container"></div>').appendTo(".ajax-container").append(player);
...
//playtoggle();// run on load U gues?
  ...
  var apiKey,
      ...
      updateTrackInfo = function($player, track) {
        // update the current track info in the player < how based on taking from data attribute?
